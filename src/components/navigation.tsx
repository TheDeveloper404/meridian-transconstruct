...
function subscribeToHash(onChange: () => void) {
  ...
  if (navigation) navigation.addEventListener("currententrychange", onChange);
  else document.addEventListener("click", afterClick);

  return () => {
    window.removeEventListener("hashchange", onChange);
    window.removeEventListener("popstate", onChange);
    if (navigation) navigation.removeEventListener("currententrychange", onChange);
...
function useActiveItem(): NavItem {
  ...
  const page = mainNav.find((item) => !item.section && item.href === pathname);
...
function NavLinks({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return mainNav.map((item) => {
    ...
    return item.section ? (
      ...
      <Link key={item.href} className="nav-link" href={item.href} aria-current={current} onClick={onNavigate}>
        {item.label}
      </Link>
    );
  });
...
export function DesktopNav() {
  return (
    <nav aria-label="Navigare principală" className="hidden items-center lg:flex xl:gap-2">
      <NavLinks />
    </nav>
  );
}
