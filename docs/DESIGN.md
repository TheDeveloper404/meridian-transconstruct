...
Două fonturi (S28, decizia utilizatorului): **Saira** pentru titluri (h1–h4), etichete (`.eyebrow`, `.kicker`), meniu și butoane — forme pătrățoase, apropiate de logo; **Inter** pentru restul textului. Ambele găzduite local prin `next/font` (A-003). Maximum trei greutăți (400, 500, 700). Scară: 16/18/22/30/42/66 px (toate dimensiunile +2 px în S18, la cererea utilizatorului; utilitarele `text-*` ale Tailwind sunt redefinite în `@theme`); titlul mare se adaptează până la 42 px pe mobil. Text curent 18 px, line-height 1.6; titluri 1.1–1.2. Logo-ul e imagine (`public/images/logo-inverse.png`, text alb pentru fundaluri închise), 48/56 px înălțime.
...
Mobil sub 768 px: o coloană, meniu nativ accesibil în machetă, fără bară fixă jos (bara „Sună” scoasă în S51); footer centrat (S49). Tabletă 768–1023 px: grile pe două coloane, meniu compact. Desktop de la 1024 px: navigare completă și hero cu imagine pe întreg fundalul. Verificare la 375, 768, 1024 și 1440 px, plus 320 px pentru overflow.
